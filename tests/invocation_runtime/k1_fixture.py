...
# The worker is a real child process. It records each run outside its
# workspace (so a re-run is observable) and commits one note.
WORKER_SCRIPT = """
...
class K1Fixture:
    ...
    def outcome_records(self) -> list[dict[str, object]]:
        return [record for record in self.journal.records() if record.get("event") == "invocation-outcome"]
