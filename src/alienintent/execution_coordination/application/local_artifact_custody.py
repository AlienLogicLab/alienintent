...
class LocalArtifactStore:
    ...
    def write(self, contents: bytes) -> CandidateRef:
        ...
        if path.exists():
            if path.read_bytes() != contents: raise ValueError("content-addressed artifact path is immutable")
...
def verify_in_fresh_process(candidate: CandidateRef) -> CandidateRef:
    completed = subprocess.run([sys.executable, "-c", "from hashlib import sha256; from pathlib import Path; import sys; print('sha256:' + sha256(Path(sys.argv[1]).read_bytes()).hexdigest())", candidate.locator], check=False, capture_output=True, text=True)
    if completed.returncode or completed.stdout.strip() != candidate.content_digest: raise ValueError("independent candidate read-back did not prove identity")
