...
class SQLiteOperationalStore(OperationalStore):
    ...
    def list_states(self, profile: str, prefix: str = "") -> tuple[tuple[str, int, dict[str, object]], ...]:
        """Read canonical state without adding a write path or schema change."""
        with self._read() as connection:
            rows = connection.execute("SELECT identity, version, state FROM aggregates WHERE profile=? AND identity LIKE ? ORDER BY identity", (profile, f"{prefix}%")).fetchall()
