...
Exit status: 0 RECONSTRUCTED/EQUAL, 1 MISMATCH, 2 HOLD.
"""
import argparse
import json
...
def compare_outputs(arguments: argparse.Namespace) -> int:
    outputs = {path: json.loads(Path(path).read_text()) for path in arguments.outputs}
    holds = sorted(path for path, output in outputs.items() if output.get("status") != "RECONSTRUCTED")
    if holds:
