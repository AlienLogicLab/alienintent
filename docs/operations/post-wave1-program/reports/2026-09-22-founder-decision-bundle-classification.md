...
| R1-GAP-MONITOR-HOST (who owns/supervises the persistent monitor host) | **RESOLVED_BY_CANONICALIZATION — proposed, pending independent check**: A6 makes persistent monitoring operational infrastructure owned by SWF-27 / SF-REQ-053; SF-REQ-053 *is* the persistent control plane host; supervision of that host is deployment authority (AA §14) exercised through installation (SF-REQ-037/038). No requirement remains unowned. If the DV reviewer disagrees, it reverts to GENUINELY_FOUNDER. | AA A6, §14; SF-REQ-053 text; FINAL-REPORT §4 table (MONITOR-HOST alone frees 5 nodes) |
