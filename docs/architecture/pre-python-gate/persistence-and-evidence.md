...
SQLite is the approved default for a simple single service; PostgreSQL supports multiple instances. Physical schema choices must preserve the same profile-scoped keys, uniqueness, expected-version updates, reservation/budget atomicity and recoverable effect intents. FD-05 requires inbox/effect-intent/outbox plus expected versions and reservation fencing, not a published schema or implemented event-sourcing framework. External effects are idempotent and use read-back/reconciliation; unknown outcomes block conflicting work rather than blind retry.
