...
## Release policy
Automatic release defaults ON and remains configurable OFF as deployment/profile policy. ON requires the configured release policy to authorize an attributable policy release: readiness evidence, satisfied dependencies, available required capabilities/budget, and no blocking ambiguity. OFF requires an explicit authorized human release event naming BIU version and current policy. Both paths create the same release command/correlation and undergo atomic admission; READY alone is never an unrestricted worker launch. Automatic release never bypasses readiness checks.
