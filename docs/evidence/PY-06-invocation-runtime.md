...
- Bounded provider smoke on this repair: `timeout 60s codex exec --ephemeral
  --json --sandbox read-only 'Reply exactly PY06_PROVIDER_SMOKE'` exited 0 and
  returned `PY06_PROVIDER_SMOKE`; reported usage was 21,038 input tokens and
  56 output tokens. Monetary cost was not reported.

## Runtime proof retained by tests

- Published source revisions are read back from a fresh clone before custody is
  admitted; an unpublished branch and a branch advertising a different revision
  are both rejected as `CandidateUnavailable`, without attempting to inspect a
  nonexistent clone directory.
...
- Producer retries wait for exponential, jittered eligibility and record the
  ...
  workspace, reserve only global capacity, retain separate provenance, and
  return `candidate-unavailable` when independent retrieval fails.
