...
**45 worktrees before, 21 after** (plus the main checkout).
...
| ACCEPTED_CANDIDATE_RETENTION | 17 | retained — candidate published to `origin`, not yet landed |
| REQUIRED_EVIDENCE_RETENTION | 2 | retained — candidate reachable from **no** remote ref |
| ORPHANED | 0 | — |
| UNKNOWN | 0 | — |
...
Node is frozen under Architecture Authority §42, so neither is changed here. Both belong to the canonical Python invocation runtime (PY-06 scope for workspace cleanup semantics; SF-REQ-024 / SWF-18 for diagnostic volume as yield evidence).
